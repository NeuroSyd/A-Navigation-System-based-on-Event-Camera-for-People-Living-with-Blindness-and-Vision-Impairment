...
import time
import csv
from denoising import _event_cube_with_denoise
...
def inference_arrows(flow_model, tensor, viz_func, mask_by_input=True, denoise_sigma=(0.1, 2.0), kernel_size=3):
    ...
    Returns:
        ...
        img (numpy.ndarray): color visualization of shape (height, width, 3) and dtype np.uint8
    """
    # denoising = GaussianBlur(kernel_size, sigma=denoise_sigma)
    # denoised_tensor = denoising(tensor)
...
def _proc(preprocessor, flow_model, mode="arrows", video_process=None,
    ...
    for tensor in tqdm(preprocessor):
        ...
        if mode == 'sharp':
            ...
            flow, img = inference_arrows(flow_model, tensor, preprocessor.get_vis_func(), mask_by_input=mask_by_input, denoise_sigma=(0.1, 2.0), kernel_size=3)
...
def main(path, delta_t, checkpoint_path, write_video="", cuda=False, mode="arrows", height=None, width=None,
    ...
    if path.endswith('h5'):
        ...
        preprocessor = CDProcessorIterator(path, "event_cube_with_denoise", delta_t=delta_t,
    ...
    process_mean = np.mean(processing_time_list)
...
def parse_args(argv=None):
    ...
    parser.add_argument('path', help='RAW, HDF5 or DAT filename, leave blank to use a camera.'
                                     'Warning if you use a HDF5 file the parameters used for precomputation must match those of'
                                     ' the model.')
    parser.add_argument('--delta-t', type=int, default=33444,
